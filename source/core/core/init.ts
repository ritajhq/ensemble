 ...
 * .ensemble/bin, copies its built-in kits, and lays out the empty
...
export async function runInit(options: RunInitOptions): Promise<void> {
  ...
  if (!compiledBinary) {
    throw new Error(`Expected a compiled ens binary in ${repoBinDir}, found none.`);
  }

  const binDir = join(ensembleDir, "bin");
  await ensureDir(binDir);
  const linkTarget = join("..", "repository", ".ensemble", "bin", compiledBinary.name);
  ...
  await copy(join(repositoryDir, ".ensemble", "kits"), join(ensembleDir, "kits"));
